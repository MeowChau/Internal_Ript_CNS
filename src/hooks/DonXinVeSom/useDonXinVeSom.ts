...
import {
  postCheckRegisterEarlyNew,
  postRegisterEarlyNew,
  getThoigianLamViec,
} from "../../apis/user";
...
export const useDonXinVeSom = () => {
  const [loading, setLoading] = useState(false);
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [selectedTime, setSelectedTime] = useState(new Date());
  const [session, setSession] = useState<"sang" | "chieu">("chieu");
  ...
  const validateForm = useCallback((): {
    ...
    // Kiểm tra lý do
    if (reason.trim() === "") {
      return { valid: false, message: "Vui lòng nhập lý do về sớm" };
    }
    ...
    const soPhutVeSom = tinhKhoangCachTheoPhut(timeSelected, thoiGianKetThuc);
    const soTongPhut = tinhKhoangCachTheoPhut(thoiGianBatDau, thoiGianKetThuc);

    // Kiểm tra thời gian về sớm phải trước giờ kết thúc
    if (soPhutVeSom <= 0) {
      return {
        valid: false,
        message: `Thời gian phải sớm hơn ${thoiGianKetThuc}`,
      };
    }

    // Kiểm tra thời gian về sớm phải sau giờ bắt đầu
    if (soPhutVeSom >= soTongPhut) {
      return {
        valid: false,
    ...
    if (soTongPhut - soPhutVeSom < 120) {
      return {
        valid: false,
        message:
          "Tổng thời gian làm việc nhỏ hơn 120 phút. Bạn nên gửi đơn xin nghỉ thay vì về sớm.",
  ...
  const checkEarlyWarning = useCallback(async (): Promise<{
    hasWarning: boolean;
    warnings: string[];
    ...
    const body = {
      ...
      phutVeSom: soPhutVeSom,
      caLam: session,
    };

    try {
      const response = await postCheckRegisterEarlyNew(body);
      const data = response?.data?.data;

      // Trường hợp nên gửi đơn nghỉ thay vì về sớm
      if (data?.value !== undefined) {
        return {
          hasWarning: true,
          warnings: [data?.message || "Nên gửi đơn xin nghỉ thay vì về sớm"],
          shouldAbsent: true,
        };
      }
  ...
  // Gửi đơn xin về sớm
  const submitEarlyRequest = useCallback(async () => {
    const thoiGianKetThuc =
      session === "sang"
        ? thoiGianLam.current.sang.thoiGianKetThuc
        : thoiGianLam.current.chieu.thoiGianKetThuc;
    ...
    setLoading(true);
    try {
      const response = await postRegisterEarlyNew(body);
      setLoading(false);
      return {
        success: true,
